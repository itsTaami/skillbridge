...
export default function BlogDetail({ blog }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <Link href="/blog" className="text-sm text-primary hover:underline mb-6 inline-block">← Back to Blog</Link>
      {blog.imgList?.[0] && <img src={blog.imgList[0]} alt={blog.title} className="w-full h-64 object-cover rounded-2xl mb-8" />}
      {blog.blogCategory && <span className="badge bg-amber-100 text-amber-700 mb-4 inline-block">{blog.blogCategory.name}</span>}
